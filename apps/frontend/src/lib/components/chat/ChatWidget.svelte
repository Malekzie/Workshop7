<script lang="ts">
    ...
    import { getThreads, createThread, getMessages, postMessage, closeThread } from '$lib/services/chat';
    ...
    function subscribeThread(t: ChatThread) {
        ...
        unsubStatus = subscribeWs(
            `/topic/chat/thread/${t.id}/status`,
            (data) => {
        ...
        {#if open}
            ...
            >
                    ...
                    <div class="flex flex-1 flex-col">
                        ...
                        <div class="border-t border-[#2C1A0E]/10 dark:border-[#FAF7F2]/10">
                            ...
                            <div class="pb-3 text-center">
                                <button
                                    onclick={() => { unsubStatus?.(); thread = null; pendingCategory = null; messages = []; }}
                ...
                {:else if pendingCategory}
                    <div class="flex shrink-0 items-center justify-between border-b border-[#2C1A0E]/10 px-4 py-2 dark:border-[#FAF7F2]/10">
                        <p class="text-xs text-[#2C1A0E]/60 dark:text-[#FAF7F2]/60">
                            Topic: <span class="font-medium capitalize text-[#2C1A0E] dark:text-[#FAF7F2]">{pendingCategory.replace('_', ' ')}</span>
                        </p>
                        <button
                            onclick={() => { pendingCategory = null; sendError = ''; }}
                    ...
                    <div class="flex flex-1 items-center justify-center">
                        <p class="px-4 text-center text-xs text-[#2C1A0E]/40 dark:text-[#FAF7F2]/40">Type your first message to start the conversation.</p>
                    </div>
                    {#if sendError}
                ...
                {:else}
                    ...
                    <div class="border-t border-[#2C1A0E]/10 px-3 py-2 text-center dark:border-[#FAF7F2]/10">
                        {#if confirmClose}
                            ...
                            {/if}
                            <p class="text-xs text-[#2C1A0E]/70 dark:text-[#FAF7F2]/70">Close this conversation?</p>
                            <div class="mt-1.5 flex justify-center gap-4">
                                ...
                                <button
                                    onclick={() => { confirmClose = false; closeError = ''; }}
                                    disabled={closing}
